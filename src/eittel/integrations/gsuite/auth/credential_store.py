...
def get_credential_store() -> CredentialStore:
    ...
    - If FIRESTORE_PROJECT is set: Uses FirestoreCredentialStore (cloud-native, multi-tenant)
    - Otherwise: Uses LocalDirectoryCredentialStore (local file storage)

    Environment Variables:
        FIRESTORE_PROJECT: GCP project ID (enables Firestore backend)
        FIRESTORE_DATABASE: Firestore database name (optional, defaults to "(default)")
        GOOGLE_MCP_CREDENTIALS_DIR: Directory for local file storage (optional)

    Returns:
        Configured credential store instance

    Raises:
        ValueError: If Firestore backend is selected but configuration is invalid
        ImportError: If Firestore backend is selected but google-cloud-firestore is not installed
    """
    ...
    if _credential_store is None:
        ...
        if firestore_project:
            # Use Firestore backend (cloud-native, multi-tenant)
            try:
                from .firestore_credential_store import FirestoreCredentialStore

                firestore_database = os.getenv("FIRESTORE_DATABASE", "(default)")

                _credential_store = FirestoreCredentialStore(
                ...
                logger.info(
                    ...
                    f"backend=FirestoreCredentialStore, "
                ...
                logger.error(
                    f"Failed to import FirestoreCredentialStore. "
                    f"Ensure google-cloud-firestore is installed: {e}"
                )
                raise ImportError(
                ...
                logger.error(f"Invalid Firestore configuration: {e}")
                raise

            except RuntimeError as e:
                # RuntimeError raised by FirestoreCredentialStore with detailed context
                logger.error(f"Firestore initialization failed: {e}")
                raise

            except Exception as e:
                logger.error(
                    f"Unexpected error initializing Firestore credential store: {e}",
                    exc_info=True,
                )
                raise RuntimeError(
                    f"Failed to initialize Firestore credential store: {e}. "
                    f"Check logs for details."
                ) from e

        else:
            # Use local file storage backend (original behavior)
            credentials_dir = os.getenv(
                "GOOGLE_MCP_CREDENTIALS_DIR",
                None,  # Let LocalDirectoryCredentialStore use its default
            )

            _credential_store = LocalDirectoryCredentialStore(base_dir=credentials_dir)

            logger.info(
                f"Initialized local file credential store: "
                f"backend=LocalDirectoryCredentialStore, "
                f"directory={_credential_store.base_dir}"
            )
